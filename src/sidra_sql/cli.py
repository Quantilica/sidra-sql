...
def install_plugin(url: str, alias: Optional[str] = typer.Option(None, help="Alias for the plugin")):
...
def update_plugin(alias: Optional[str] = typer.Argument(None, help="Alias of the plugin to update (updates all if omitted)")):
...
def remove_plugin(alias: str = typer.Argument(..., help="Alias of the plugin to remove")):
...
def run_pipeline(
    ...
    force_metadata: bool = typer.Option(False, "--force-metadata", help="Force refresh metadata")
):
    ...
    try:
        ...
        console.print(f"[bold blue]Running pipeline {pipeline_id} from {alias}[/bold blue]")
        
        # Run Fetch
        console.print("[blue]-> Running Fetch...[/blue]")
        toml_script = TomlScript(config, pipeline.fetch, force_metadata=force_metadata)
        toml_script.run()
